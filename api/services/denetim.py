...
def rapor_verisi(db: Session, gun: date) -> list[dict]:
    """Dün (yerel gün) en az bir kaydı olan gerçek bebeklerin bölümleri."""
    ...
    kayitli = {bid for (bid,) in (db.query(SleepLog.baby_id)
                                  .filter(gun_filtresi(gun)).distinct().all())}
    bolumler = []
...
def html_uret(gun: date, bolumler: list[dict]) -> str:
    ...
    for b in bolumler:
        ...
        parcalar.append(
            f'<section><h2>Bebek {b["no"]}, {html.escape(b["yas_metni"])}</h2>'
    ...
    govde = "".join(parcalar) or '<p class="bos">Dün kaydı olan beta bebeği yok.</p>'
    toplam_geri = sum(len(b["geri_bildirim"]) for b in bolumler)
...
<p class="alt">{len(bolumler)} bebek · {toplam_geri} geri bildirim · saatler Türkiye saati ·
kişisel veri içermez</p>
...
def rapor_yaz(db: Session, gun: date) -> tuple[Path, str, int]:
    ...
    gecici.write_text(html_uret(gun, bolumler), encoding="utf-8")
    gecici.replace(yol)
    return yol, imzali_baglanti(gun), len(bolumler)
...
def gunluk_is() -> None:
    """APScheduler işi — her sabah 08:00 (TR). Çok worker'da YALNIZ BİRİ üretir
    (Postgres advisory lock); kilidi alamayan sessizce çıkar."""
    ...
    try:
        ...
        try:
            gun = dun()
            yol, baglanti, n = rapor_yaz(db, gun)
            logger.info("Günlük denetim hazır: %s · %d bebek · %s", gun, n, baglanti)
        finally:
            db.close()
    except Exception:
        logger.exception("Günlük denetim üretilemedi")
    finally:
        if conn is not None:
            try:
